...
const ChatContainer = ()=>{
    ...
    //Handle sending ans image
    const handleSendImage = async(e)=>{
        const file = e.target.files[0];
        //dont have file || selected file is not the img
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg , index)=>(
            // Message दुसऱ्या user कडून आले असेल
            <div key = {index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}> 
                ...
                {/* this div for img & time in messages */}
      ...
      {/* ---------------------Bottom Area------------ */}
       <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
            ...
            value = {input} onKeyDown={()=>e.key === "Enter" ? handleSendMessage(e) : null} type="text" placeholder='Send a message' 
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500 bg-white/10 max-sm:hidden'>
